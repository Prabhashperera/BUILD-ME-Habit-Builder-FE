...
import { Mail, Lock, User, ArrowRight, Github, Sparkles, Trophy } from "lucide-react";
...
const SignupPage = () => {
    ...
    const handleGoogleLogin = async (credentialResponse: any) => {
        try {
            ...
            navigate("/habits");
        } catch (err) { console.error(err); toast.error("Google signup failed"); }
    };
            ...
            <div className="hidden lg:flex w-1/2 relative items-center justify-center overflow-hidden bg-[#050505]">
                ...
                <div className="relative z-10 max-w-lg p-12">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/10 bg-white/5 backdrop-blur-md mb-8 shadow-2xl">
                        ...
                        <span className="text-xs font-medium tracking-widest text-white/80 uppercase">New Profile</span>
                    </div>
                    
                    <h1 className="text-6xl font-medium text-white mb-6 tracking-tight leading-tight">
                        Level up your <br />
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-300 to-white">real life stats.</span>
                    </h1>
                    
                    <p className="text-lg text-white/40 mb-12 leading-relaxed font-light">
                        Join the protocol. Build consistency. Unlock your potential.
                    </p>
                    ...
                    <div className="group relative bg-white/5 border border-white/10 p-6 rounded-3xl backdrop-blur-xl hover:bg-white/10 transition-colors duration-500">
                        <div className="absolute -inset-0.5 bg-gradient-to-r from-amber-500/20 to-orange-500/20 rounded-3xl blur opacity-0 group-hover:opacity-100 transition duration-500" />
                        <div className="relative flex items-center gap-5">
                            <div className="p-3.5 rounded-2xl bg-amber-500/20 text-amber-300">
                                <Trophy className="w-6 h-6" />
                            </div>
                            <div>
                                <div className="text-xs font-bold text-white/30 uppercase tracking-widest mb-1">Achievement Unlocked</div>
                                <div className="text-2xl font-medium text-white">Early Adopter <span className="text-sm text-amber-400 ml-2 font-normal">+500 XP</span></div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-8 relative bg-black">
                <div className="w-full max-w-md space-y-10">
                    ...
                    <div className="text-center lg:text-left space-y-2">
                        ...
                        <p className="text-white/40 text-sm">Initialize your personal dashboard.</p>
                    </div>
                    ...
                    <form onSubmit={handleSignUp} className="space-y-5">
                        ...
                        <div className="group space-y-2">
                            <label className="text-xs font-medium text-white/40 uppercase tracking-wide ml-1">Username</label>
                            <div className="relative">
                                <User className="absolute left-4 top-3.5 h-5 w-5 text-white/20 group-focus-within:text-white transition-colors" />
                                <input
                                    type="text"
                                    value={userName}
                        ...
                        {/* Email */}
                        <div className="group space-y-2">
                            <label className="text-xs font-medium text-white/40 uppercase tracking-wide ml-1">Email</label>
                            <div className="relative">
                        ...
                        <button
                            type="submit"
                            disabled={isLoading}
                            className="w-full group relative overflow-hidden bg-white text-black py-3.5 rounded-xl font-medium text-sm transition-all hover:bg-white/90 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed mt-4"
                        >
                            <span className="relative z-10 flex items-center justify-center gap-2">
                                {isLoading ? "Initializing..." : <>Get Started <ArrowRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" /></>}
                            </span>
                        </button>
                    </form>
                    ...
                    <div className="relative">
                        ...
                        <div className="relative flex justify-center">
                            <span className="px-4 bg-black text-xs text-white/20 uppercase tracking-widest">Or join with</span>
                        </div>
                    </div>
                    ...
                    <p className="text-center text-sm text-white/40">
                        ...
                        <Link to="/login" className="font-medium text-white hover:underline decoration-white/30 underline-offset-4 transition-all">
                            Log in
                        </Link>
                    </p>
                </div>
