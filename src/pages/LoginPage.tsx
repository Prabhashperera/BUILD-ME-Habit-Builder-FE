...
const LoginPage = () => {
    ...
    // 2. UPDATED LOGIN FUNCTION (Async + Unwrap)
    const handleLogin = async (e: any) => {
        // This prevents the page refresh!
        e.preventDefault();

        try {
            // We wait for the action to complete using .unwrap()
            ...
            // We use 'result' directly instead of waiting for 'state.data'
            ...
            // Error toast is handled by the useEffect below
        }
    }

    // 3. REMOVED the useEffect that was watching [data]. 
    // Navigation is now handled inside handleLogin above.
            ...
            <div className="relative w-full max-w-4xl bg-slate-900/80 backdrop-blur-xl border border-emerald-500/20 rounded-3xl shadow-[0_0_40px_-10px_rgba(16,185,129,0.3)] overflow-hidden grid lg:grid-cols-5">
                ...
                <div className="hidden lg:flex lg:col-span-2 flex-col justify-center items-center p-12 bg-linear-to-br from-emerald-900/40 via-slate-900/80 to-teal-900/40 relative overflow-hidden">
                    ...
                    <div className="relative z-10 text-center space-y-6">
                        ...
                        <h2 className="text-3xl font-extrabold text-white tracking-tight">Keep The <br /> <span className="text-emerald-400">Streak Alive</span></h2>
                        <p className="text-emerald-100/60 text-sm max-w-xs mx-auto leading-relaxed">Consistency is the key. Log in to track your daily wins and evolve your avatar.</p>
                        ...
                        <div className="mt-10 p-3 bg-slate-950/60 rounded-xl border border-emerald-500/20 w-full max-w-[220px] flex items-center gap-4 shadow-lg transform hover:scale-105 transition-transform cursor-default">
                            <div className="bg-linear-to-br from-orange-500 to-red-500 p-2.5 rounded-lg shadow-orange-500/20 shadow-lg"><Zap className="w-5 h-5 text-white fill-white" /></div>
                            <div className="text-left">
                                <div className="text-[10px] text-slate-400 uppercase font-bold tracking-wider">Current Streak</div>
                                <div className="text-lg font-mono text-white font-bold leading-none mt-1">12 Days <span className="text-emerald-500 text-xs ml-1">▲</span></div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-8 md:p-12 lg:col-span-3 flex flex-col justify-center relative">
                    ...
                    {/* FIX 1: Added onSubmit={handleLogin} here. 
                        This catches the "Enter" key and stops the refresh.
                    */}
                    <form className="space-y-5" onSubmit={handleLogin}>

                        {/* Email Input */}
                        <div className="group">
                            <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 group-focus-within:text-emerald-400 transition-colors">Email</label>
                        ...
                        <div className="group">
                            <div className="flex justify-between items-center mb-2">
                                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider group-focus-within:text-emerald-400 transition-colors">Password</label>
                        ...
                        >
                            <span>{isLoading ? "Logging in..." : "Log Workout / Habit"}</span>
                            <ChevronRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
                        </button>
                    </form>

                    {/* Divider, Socials, Footer (Keep as is) */}
                    <div className="relative my-6">
                        <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-slate-800"></div></div>
                        <div className="relative flex justify-center text-xs uppercase tracking-wider"><span className="px-4 bg-slate-900 text-slate-500">Login with</span></div>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <button type="button" className="flex items-center justify-center gap-2 p-3 bg-slate-800/50 hover:bg-slate-800 border border-slate-700 hover:border-slate-600 rounded-lg text-slate-300 transition-all group cursor-pointer">
                            <Github className="w-5 h-5 group-hover:text-white transition-colors" /><span className="text-sm font-semibold">Github</span>
                        </button>
                        <button type="button" className="flex items-center justify-center gap-2 p-3 bg-slate-800/50 hover:bg-slate-800 border border-slate-700 hover:border-slate-600 rounded-lg text-slate-300 transition-all group cursor-pointer">
                            <Mail className="w-5 h-5 text-sky-500/70 group-hover:text-sky-400 transition-colors" /><span className="text-sm font-semibold">Google</span>
                        </button>
                    </div>
                    <Link to={"/signup"} >
                        <p className="mt-8 text-center text-slate-500 text-sm">Ready to start growing? <span className="text-emerald-400 hover:text-emerald-300 font-bold hover:underline transition-colors">Start your journey</span></p>
                    </Link>
                </div>
            </div>
        </div>
